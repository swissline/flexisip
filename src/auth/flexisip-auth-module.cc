...
FlexisipAuthModule::FlexisipAuthModule(su_root_t *root, const std::string &domain, const std::string &algo):
AuthModule(root,
...
FlexisipAuthModule::FlexisipAuthModule(su_root_t *root, const std::string &domain, const std::string &algo, int nonceExpire):
AuthModule(root,
		   AUTHTAG_REALM(domain.c_str()),
		   AUTHTAG_OPAQUE("+GNywA=="),
		   AUTHTAG_QOP("auth"),
...
void FlexisipAuthModule::onCheck(AuthStatus &as, msg_auth_t *au, auth_challenger_t const *ach) {
...
void FlexisipAuthModule::onCancel(AuthStatus &as) {
...
void FlexisipAuthModule::flexisip_auth_check_digest(FlexisipAuthStatus &as, msg_auth_t *au, auth_challenger_t const *ach) {
 ...
 */
void FlexisipAuthModule::finish(FlexisipAuthStatus &as) {
...
void FlexisipAuthModule::onError(FlexisipAuthStatus &as) {
	if (as.status() != 0) {
		as.status(500);
